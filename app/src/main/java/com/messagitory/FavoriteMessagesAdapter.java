...
public class FavoriteMessagesAdapter extends BaseAdapter {
    ...
    private void loadData(final ViewHolder holder, final int position) {
        ...
        holder.edit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ((FavoriteMessages) mActivity).showDialogMessage(false, messages.get(position).getMessage(), "");
            }
        });
                ...
                alertDialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        if (messages.get(position).getMessage().length() > 240) {
                            ...
                            ((FavoriteMessages) mActivity).addMessage(messages.get(position).getMessage());
                        }
                    }
                });
                alertDialog.show();
